...
class GetPhotosOperation: GroupOperation {
  
  // MARK: Properties
  let downloadOperation: DownloadPhotosOperation
  let parseOperation: ParsedPhotoOperation
  ...
  init(realm: Realm, completionHandler: Void -> Void) {
    ...
    let cacheFile = cachesFolder.URLByAppendingPathComponent("photos.json")
    ...
    */
    ...
    downloadOperation = DownloadPhotosOperation(cacheFile: cacheFile)
    parseOperation = ParsedPhotoOperation(cacheFile: cacheFile)
